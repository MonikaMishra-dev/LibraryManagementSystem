...
public class BookTitleFilterImpl implements BookFilterStrategy {
    ...
    public List<BookFilterResponse> getFilteredBook(Operator operator, String value) {
        ...
        if (operator.equals(Operator.EQUALS))
            bookList = bookRepository.findByTitle(value);
        else if (operator.equals(Operator.CONTAINS))
            bookList = bookRepository.findByTitleContains(value);
        else if (operator.equals(Operator.LIKE))
            bookList = bookRepository.findByTitleLike("%" + value + "%");
        else
            throw new IllegalArgumentException("Only Equals, Contains, Like is allowed for bok name");
